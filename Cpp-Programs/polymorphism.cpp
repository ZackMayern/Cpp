...
set() - Set/insert the value of the key, if present, otherwise add the key as the most recently used key. If the cache has reached its capacity, it should replace the least recently used key 
with a new key.
...
An input line starting with get will be followed by a key to be found in the cache. An input line starting with set will be followed by the key and and respectively to be inserted/replaced in 
the cache.
*/
...
class LRUCache : public Cache{
    ...
    void set(int a, int b){   //a=key and b=value
        Node *N;
        if(mp.empty()){
            N = new Node(a,b);
            tail = head = N;
